...
class Word2Vec:
    ...
    def train(self):
        ...
        for i in process_bar:
            ...
            loss.backward()
            clip_grad_norm(self.skip_gram_model.parameters(), max_norm=5)
            self.optimizer.step()
            ...
            if i != 0 and i % count == 0:
                self.skip_gram_model.save_embedding(self.data.id2word,self.output_file_name + str(i) +'_'+ str(self.initial_lr))
        self.skip_gram_model.save_embedding(self.data.id2word, self.output_file_name+'_'+ str(self.initial_lr))


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--input_file", default="./data.txt_plain", type=str)
    parser.add_argument("--output_file", default="./data.txt_plain.emb300.txt", type=str)
